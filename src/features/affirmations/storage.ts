...
const BACKGROUND_PREFERENCE_STORAGE_KEY =
  '@moodie/affirmation-background-preference';
...
function isAffirmationTopicId(value: unknown): value is AffirmationTopicId {
  return typeof value === 'string' && value.trim().length > 0;
}

function isAffirmationBackgroundId(
  value: unknown,
): value is NonNullable<AffirmationBackgroundPreference['backgroundId']> {
  return typeof value === 'string' && value.trim().length > 0;
}

function isAffirmationBackgroundMode(
  value: unknown,
): value is AffirmationBackgroundMode {
  return value === 'free' || value === 'fixed';
}

function getUniqueTopicIds(
...
> {
  ...
  const validTopicIds = getUniqueTopicIds(
    parsedValue.filter(isAffirmationTopicId),
  );
...
export async function saveAffirmationBackgroundPreference(
  ...
  const hasValidBackgroundId =
    preference.backgroundId === null ||
    isAffirmationBackgroundId(preference.backgroundId);
...
export async function saveLikedAffirmationKeys(
  likeKeys: string[],
): Promise<void> {
...
export async function toggleLikedAffirmationKey(
  likeKey: string,
): Promise<boolean> {
